...
auth.onAuthStateChanged((user) => {
    ...
    if (user || isTester) {
        ...
        // B. switchTab('home') DIHAPUS dari sini — sudah ditangani oleh initApp()
        // Memanggil switchTab di sini menyebabkan loadLatest() jalan dua kali (race condition)
                ...
                let historyHtml = (historyData && historyData.length > 0) ? historyData.map(item => {
                    ...
                    <div onclick="loadDetail('${item.url}')" style="display:flex; gap:15px; background:#1c1c1e; padding:12px; border-radius:16px; border:1px solid #2c2c2e; margin-bottom:12px; cursor:pointer;">
                        ...
                        <div style="flex:1; min-width:0; display:flex; flex-direction:column; justify-content:center;">
                            <div style="font-weight:800; font-size:14px; color:#fff; margin-bottom:6px; display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical; overflow:hidden;">${item.titleEnglish||item.title}</div>
...
window.renderDetailEpisodeUI = function() {
    ...
    let sortText = window.epSortOrder === 'desc' ? 'Terbaru &#9660;' : 'Terlama &#9650;';
...
function getEpBadge(anime) {
    ...
    // Prioritas 1: ambil angka terbesar dari semua episode di array
    if (Array.isArray(anime.episodes) && anime.episodes.length > 0) {
        let maxEp = 0;
        for (const ep of anime.episodes) {
            const str = ep.title || ep.url || '';
        ...
        if (maxEp > 0) return `Episode ${maxEp}`;
        // Kalau array ada tapi angka 0 semua, pakai length
        return `Episode ${anime.episodes.length}`;
    }
    // Prioritas 2: field episode string
    let text = String(anime.episode || '');
    if (text && text !== 'undefined' && text.trim() !== '' && text.trim() !== '0') {
        ...
        if (epMatch) return `Episode ${epMatch[2]}`;
        let numMatch = text.match(/\d+/g);
        if (numMatch) {
            const n = parseInt(numMatch[numMatch.length - 1]);
            if (n > 0) return `Episode ${n}`;
        }
    }
    // Prioritas 3: status
    let status = String(anime.status || '').toLowerCase();
    if (status.includes('tamat') || status.includes('completed') || status.includes('finished')) return 'Tamat';
    if (status.includes('movie')) return 'Movie';
    return 'Ongoing';
}
...
async function toggleFavorite(url, title, image, score, episode) {
    try {
        ...
        // Helper: simpan/hapus dari IndexedDB dengan promise yang bener
        const localWrite = (action) => new Promise(async (res, rej) => {
            try {
                const database = await initDB();
                const tx = database.transaction(STORE_FAV, 'readwrite');
                const store = tx.objectStore(STORE_FAV);
                if (action === 'put') store.put(favData);
    ...
    return `
        <div style="width:160px;min-width:160px;cursor:pointer;display:flex;flex-direction:column;scroll-snap-align:start;border-radius:10px;overflow:hidden;background:#111;flex-shrink:0;" onclick="loadDetail('${anime.url}')">
            <div style="position:relative;width:100%;padding-top:56.25%;background:#1a1a1a;overflow:hidden;">
                <img src="${getHighRes(anime.image)}" alt="${anime.titleEnglish||anime.title}" loading="lazy" onerror="${fallbackImg}" style="position:absolute;top:0;left:0;width:100%;height:100%;object-fit:cover;">
            ...
            <div style="padding:7px 8px 8px;display:flex;flex-direction:column;gap:3px;">
                <div style="font-size:11px;color:#e4e4e7;font-weight:600;line-height:1.35;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;word-break:break-word;">${anime.titleEnglish||anime.title}</div>
...
// Helper: bikin HTML ghost card biar grid selalu kelipatan 3
function padGridTo3(cardsHtml, count) {
    const rem = count % 3;
    if (rem === 0) return cardsHtml;
    const ghost = '<div style="background:transparent;border-radius:8px;padding-top:148%;"></div>';
    return cardsHtml + ghost.repeat(3 - rem);
}
...
function generateCardHtml(anime) {
    ...
    // FIX v108: Sembunyikan anime yang belum punya episode sama sekali
    const isFromMal = anime.source === 'myanimelist';
    const hasNoEpisode = (
        epsBadge === 'Episode ?' ||
        epsBadge === '?' ||
        // Dari API: field hasEpisodes explicit false = tidak ada data
        (anime.hasEpisodes === false) ||
        // Array episodes kosong
        (Array.isArray(anime.episodes) && anime.episodes.length === 0 && !anime.streamSource) ||
        // episode_count nol
        (anime.episode_count !== undefined && (anime.episode_count === '0' || anime.episode_count === 0) && !anime.streamSource)
    );
    // Anime dari MAL (top/genre) yang belum ada episode: sembunyikan total
    if (isFromMal && hasNoEpisode) return '';
    // Anime dari Samehadaku tanpa episode dan tanpa streamSource: sembunyikan
    if (!isFromMal && hasNoEpisode && !anime.streamSource && !anime.url?.includes('samehadaku')) return '';
    ...
    return `
        <div class="anime-big-card" onclick="${clickAction}" style="${dimStyle}">
            <div class="anime-big-card-img">
                <img src="${getHighRes(anime.image)}" alt="${anime.titleEnglish||anime.title}" loading="lazy" onerror="${fallbackImg}">
                ${unavailableBadge}
            ...
            <div style="padding: 6px 2px;">
                <div class="anime-big-card-views">
                    ...
                    ${mockViews}${anime.type ? ` <span style="background:#1e293b;color:#94a3b8;font-size:10px;font-weight:700;padding:1px 5px;border-radius:4px;margin-left:4px;">${anime.type}</span>` : ''}
                </div>
                <div class="anime-big-card-title">${anime.titleEnglish||anime.title}</div>
            </div>
        </div>`; 
}
...
// FIX v104: in-memory API cache biar /api/latest dan /api/top-anime tidak double-fetch
// TTL 2 menit — cukup untuk 1 sesi browsing tanpa stale data
window._apiMem = window._apiMem || {};
async function fetchMem(url, timeoutMs = 15000) {
    const now = Date.now();
    const TTL = 120000; // 2 menit
    if (window._apiMem[url] && (now - window._apiMem[url].ts) < TTL) {
        // return fake Response object yang compatible dengan .ok dan .json()
        return { ok: true, json: async () => window._apiMem[url].data };
    }
    const res = await fetchTimeout(url, timeoutMs);
    if (res && res.ok) {
        const data = await res.json();
        window._apiMem[url] = { ts: now, data };
        ...
        if(searchContainer) {
            searchContainer.innerHTML = `
                ...
                <div class="anime-grid-2col">
                    ${padGridTo3(data.map(anime => generateCardHtml(anime)).join(""), data.length)}
                </div>
            `;
        }
    } catch (err) {
...
// State untuk infinite scroll genre
window._genreState = { name:'', page:1, totalPages:1, loading:false };

window.loadGenre = async function(genreName, reset) {
    reset = reset !== false; // default true (fresh load)
    if(reset) {
        window._genreState = { name: genreName, page: 1, totalPages: 1, loading: false };
    ...
    if(window._genreState.loading) return;
    window._genreState.loading = true;
    const page = window._genreState.page;
    try {
        const res = await fetch(`${API_BASE}/genre?name=${encodeURIComponent(genreName)}&page=${page}&limit=200`);
        const json = await res.json();
        const animes = json.data || [];
        const total = json.total || 0;
        window._genreState.totalPages = json.totalPages || 1;
        let searchContainer = document.getElementById('search-view');
        if(!searchContainer) return;
        if(reset) {
            // Render fresh
            const gridId = 'genre-grid-' + Date.now();
            searchContainer.innerHTML = `
                <div class="header-flex" style="padding-top:20px;">
                    <h2>Genre: ${genreName}</h2>
                    <span style="color:#666; font-size:13px; font-weight:700;">Total (${total})</span>
                </div>
                <div id="${gridId}" class="anime-grid-2col">
                    ${animes.length > 0 ? animes.map(a => generateCardHtml(a)).join('') : '<div style="padding:40px 15px; text-align:center; color:#555; grid-column:1/-1;">Tidak ada anime ditemukan untuk genre ini.</div>'}
                </div>
                <div id="genre-scroll-sentinel" style="height:40px;"></div>
            `;
            searchContainer.dataset.gridId = gridId;
            // Setup infinite scroll observer
            if(window._genreObserver) window._genreObserver.disconnect();
            window._genreObserver = new IntersectionObserver((entries) => {
                if(entries[0].isIntersecting && !window._genreState.loading && window._genreState.page < window._genreState.totalPages) {
                    window._genreState.page++;
                    window.loadGenre(window._genreState.name, false);
                }
            }, { threshold: 0.1 });
            const sentinel = document.getElementById('genre-scroll-sentinel');
            if(sentinel) window._genreObserver.observe(sentinel);
        } else {
            // Append ke grid yang sudah ada
            const gridId = searchContainer.dataset.gridId;
            const grid = document.getElementById(gridId);
            if(grid && animes.length > 0) {
                grid.insertAdjacentHTML('beforeend', animes.map(a => generateCardHtml(a)).join(''));
            }
        }
    } catch (err) {
        if(reset) {
        ...
        window._genreState.loading = false;
        if(reset) loader(false);
    }
};

window.loadTopAnime = async function() {
    if (window.location.hash !== '#search') history.pushState({page: 'search'}, '', '#search');
    switchTab('search');
    loader(true);
    let searchContainer = document.getElementById('search-view');
    if(searchContainer) searchContainer.innerHTML = `<div class="header-flex" style="padding-top:20px;"><h2>Top Anime</h2></div><div class="anime-grid-2col" style="opacity:0.4;">${Array(9).fill('<div style="background:#111;border-radius:8px;padding-top:148%;"></div>').join('')}</div>`;
    try {
        const res = await fetchMem(`${API_BASE}/top-anime?limit=100`, 12000);
        ...
        if(searchContainer) {
            searchContainer.innerHTML = `
                ...
                <div class="anime-grid-2col">
                    ${data.length > 0 ? padGridTo3(data.map(a => generateCardHtml(a)).join(""), data.length) : '<div style="padding:40px 15px; text-align:center; color:#555; grid-column:1/-1;">Tidak ada data.</div>'}
                </div>`;
        }
    } catch(e) {
...
window.loadAllAnime = async function() {
    ...
    try {
        // FIX v104: pakai fetchMem biar pakai cache kalau ada
        const res = await fetchMem(`${API_BASE}/latest`, 12000);
        const data = res && res.ok ? await res.json() : [];
        if(searchContainer) {
            searchContainer.innerHTML = `
                <div class="header-flex" style="padding-top:20px;">
...
window.loadAllTopAnime = async function() {
    if (window.location.hash !== '#search') history.pushState({page: 'search'}, '', '#search');
    switchTab('search');
    loader(true);
    let searchContainer = document.getElementById('search-view');
    if(searchContainer) searchContainer.innerHTML = `<div class="header-flex" style="padding-top:20px;"><h2>Top Anime</h2></div><div class="anime-grid-2col" style="opacity:0.4">${Array(12).fill('<div style="background:#111;border-radius:8px;padding-top:148%;"></div>').join('')}</div>`;
    try {
        const res = await fetchMem(`${API_BASE}/top-anime?limit=100`, 12000);
        const data = res && res.ok ? await res.json() : [];
        if(searchContainer) {
...
async function loadLatest() {
    // GUARD: Cegah double-load jika sudah loading (race condition antara initApp & onAuthStateChanged)
    if (window._homeLoading) return;
    ...
    try {
        // FIX v111: Fetch latest + top-anime PARALLEL — tidak sequential lagi
        const [latestRes, topRes] = await Promise.allSettled([
            fetchMem(`${API_BASE}/latest`, 12000),
            fetchMem(`${API_BASE}/top-anime?limit=60`, 12000),
        ]);
        const _latestData = (latestRes.status === 'fulfilled' && latestRes.value?.ok) ? await latestRes.value.json() : [];
        const _topData    = (topRes.status === 'fulfilled'    && topRes.value?.ok)    ? await topRes.value.json()    : [];
        ...
        // Hero slider pakai data yang sudah ada (tidak fetch lagi)
        if (_latestData && _latestData.length > 0) {
            try { renderHeroSlider(_latestData.slice(0, 20), homeContainer); } catch(sliderErr) { console.error('Slider error:', sliderErr); }
            hasAnyData = true;
        }
        ...
        // Anime Update - pakai _latestData yang sudah ada, tanpa fetch lagi
        if (Array.isArray(_latestData) && _latestData.length > 0) {
            const freshData = _latestData.filter(a => {
                const epText = String(a.episode || a.status || a.type || '').toLowerCase();
                return !epText.includes('tamat') && !epText.includes('completed') && !epText.includes('movie');
            });
            const displayData = freshData.length > 0 ? freshData : _latestData;
            const animeUpdateDiv = document.createElement('div');
            animeUpdateDiv.innerHTML = `<div class="header-flex"><h2>Anime Update</h2><span class="more-link" onclick="window.loadAllAnime()">Selengkapnya ›</span></div><div class="anime-grid-2col">${padGridTo3(displayData.slice(0, 18).map(a => generateCardHtml(a)).join(""), Math.min(displayData.length, 18))}</div>`;
        ...
        const GENRE_LIST = [
            ...
            "Fantasy", "Game", "Girls Love", "Boys Love", "Gore",
            "Harem", "Historical", "Horror", "Isekai", "Josei",
            "Magic", "Mahou Shoujo", "Martial Arts", "Mecha", "Military",
            "Music", "Mystery", "Parody", "Psychological", "Racing",
            ...
            "Time Travel", "Vampire", "Video Game",
        ];
        ...
        // Top Anime - pakai _topData yang sudah ada, tanpa fetch lagi
        if (Array.isArray(_topData) && _topData.length > 0) {
            const topAnimeHomeDiv = document.createElement('div');
            topAnimeHomeDiv.innerHTML = `
                <div style="margin-top: 10px;">
                    <div class="header-flex">
                        <h2 style="margin:0;">Top Anime</h2>
                        <span class="more-link" onclick="window.loadAllTopAnime()">Selengkapnya ›</span>
                    </div>
                    <div class="anime-grid-2col">${padGridTo3(_topData.slice(0, 36).map(a => generateCardHtml(a)).join(""), Math.min(_topData.length, 36))}</div>
                </div>
            `;
            homeContainer.appendChild(topAnimeHomeDiv);
            hasAnyData = true;
        }
    ...
    } catch (err) { console.error("Home loading failed total", err); } finally { loader(false); window._homeLoading = false; }
}
...
window.handleDownload = function() { let iframe = document.getElementById('video-player'); if(iframe && iframe.src && iframe.src !== 'about:blank' && iframe.src !== window.location.href) { window.open(iframe.src, '_blank'); } else { window.showToast('Tidak ada video untuk didownload.', 'error'); } };

window.retryLoadStream = async function(url) {
    const container = document.querySelector('.video-container-fixed');
    if(!container) return;
    container.innerHTML = `
        ...
        <div style="position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;background:#000;">
            <div style="width:40px;height:40px;border:3px solid rgba(59,130,246,0.2);border-top-color:#3b82f6;border-radius:50%;animation:spin 1s linear infinite;margin-bottom:12px;"></div>
            <div style="color:#a1a1aa;font-size:13px;font-weight:600;">Mencari stream...</div>
        </div>
    `;
    try {
        const targetUrl = url || (window.currentPlayingAnime && window.currentPlayingAnime.url);
        if(!targetUrl) throw new Error('no url');
        if(window.apiCache) delete window.apiCache['watch_' + targetUrl];
        const res = await fetch(`${API_BASE}/watch?url=${encodeURIComponent(targetUrl)}`);
        const data = await res.json();
        if(data.streams && data.streams.length > 0) {
            container.innerHTML = `
                <button class="watch-back-btn" onclick="backToDetail()"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M19 12H5M12 19l-7-7 7-7"/></svg></button>
                <iframe id="video-player" src="${data.streams[0].url}" allowfullscreen allow="autoplay; fullscreen; encrypted-media" frameborder="0" style="position:absolute;top:0;left:0;width:100%;height:100%;border:none;display:block;"></iframe>
                <div id="video-loading-overlay" style="position:absolute;inset:0;background:#000;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:5;pointer-events:none;transition:opacity 0.5s;">
                    <div style="width:40px;height:40px;border:3px solid rgba(59,130,246,0.2);border-top-color:#3b82f6;border-radius:50%;animation:spin 1s linear infinite;margin-bottom:12px;"></div>
                    <div style="color:#a1a1aa;font-size:13px;font-weight:600;">Memuat video...</div>
                </div>
            `;
            ...
            if(modalServerContainer) {
                modalServerContainer.innerHTML = data.streams.map((stream, idx) => {
                    return `<button class="${idx===0?'server-list-btn active':'server-list-btn'}" onclick="changeServer('${stream.url}', '${stream.server}', this)"><span>${stream.server}</span></button>`;
                }).join('');
            }
            window.showToast('Stream ditemukan!', 'success');
        } else {
            container.innerHTML = `
                <button class="watch-back-btn" onclick="backToDetail()"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M19 12H5M12 19l-7-7 7-7"/></svg></button>
                <div style="position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;background:#0a0a0a;">
                    <div style="font-size:40px;margin-bottom:16px;">📡</div>
                    <div style="color:#ef4444;font-size:14px;font-weight:800;margin-bottom:8px;text-align:center;">Stream Masih Belum Tersedia</div>
                    <div style="color:#6b7280;font-size:12px;margin-bottom:20px;text-align:center;line-height:1.5;padding:0 20px;">Coba tunggu beberapa menit lagi.</div>
                    <button onclick="retryLoadStream('${targetUrl}')" style="background:#3b82f6;color:#fff;border:none;padding:10px 24px;border-radius:20px;font-size:13px;font-weight:800;cursor:pointer;">🔄 Coba Lagi</button>
                    <div id="video-player" style="display:none;"></div>
                </div>
            `;
        }
    } catch(e) { window.showToast('Gagal retry stream.', 'error'); }
};
    ...
    try {
        ...
        for (const [dateLabel, animes] of Object.entries(groupedData)) {
            ...
            animes.forEach(anime => {
                ...
                const progress = Math.floor(Math.random() * 70 + 20); const durasiDetik = 24 * 60; const currentDetik = Math.floor((progress/100) * durasiDetik);
                const fmtTime = (s) => `${String(Math.floor(s/60)).padStart(2,'0')}:${String(s%60).padStart(2,'0')}`;
                const currentStr = `${fmtTime(currentDetik)} / ${fmtTime(durasiDetik)}`;
                const isMulti = anime.url && (anime.url.includes('batch') || anime.url.includes('all'));
                const singleMultiBadge = `<div style="display:flex;gap:6px;margin-top:8px;"><span style="background:#3b82f6;color:#fff;font-size:10px;font-weight:800;padding:4px 10px;border-radius:20px;">SINGLE</span><span style="background:#2c2c2e;color:#666;font-size:10px;font-weight:800;padding:4px 10px;border-radius:20px;">MULTI</span></div>`;
                const fallbackImg = "this.src='https://placehold.co/160x90/1a1a1a/3b82f6?text=Anime'";
                timelineHtml += `<div class="timeline-card" onclick="loadDetail('${anime.url}')"><div class="timeline-img"><img src="${getHighRes(anime.image)}" alt="${anime.titleEnglish||anime.title}" onerror="${fallbackImg}"><div class="timeline-play-icon"><svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg></div></div><div class="timeline-info"><div class="timeline-header"><div class="timeline-title">${anime.titleEnglish||anime.title}</div><div class="timeline-time">${timeStr}</div></div><div class="timeline-ep">${getEpBadge(anime)}</div><div class="timeline-progress-container"><div class="timeline-progress-bg"><div class="timeline-progress-fill" style="width: ${progress}%;"></div></div><div class="timeline-progress-text">${currentStr}</div></div>${singleMultiBadge}</div></div>`;
            });
...
function generateFavCardHtml(anime) {
    if (!anime || !anime.title || !anime.url) return '';
    const fallbackImg = "this.src='https://placehold.co/150x225/1a1a1a/3b82f6?text=Anime'";
    let scoreStr = anime.score || '';
    let finalScore = (scoreStr && scoreStr !== '?' && scoreStr !== '0' && scoreStr !== '') ? scoreStr : '';
    let epsBadge = anime.episode || '';
    return `
        <div class="anime-big-card" onclick="loadDetail('${anime.url}')">
            <div class="anime-big-card-img">
                <img src="${anime.image || ''}" alt="${anime.titleEnglish||anime.title}" loading="lazy" onerror="${fallbackImg}">
                ${epsBadge ? `<div class="badge-ep">${epsBadge}</div>` : ''}
                ${finalScore ? `<div class="badge-score"><svg width="10" height="10" viewBox="0 0 24 24" fill="#fbbf24"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg> ${finalScore}</div>` : ''}
            </div>
            <div style="padding: 6px 2px;">
                <div class="anime-big-card-title">${anime.titleEnglish||anime.title}</div>
            </div>
...
async function loadDetail(url) {
    // FIX v44: simpan konten search view supaya back bisa restore
    const sc = document.getElementById('search-view');
    if (sc) window._savedSearchContent = sc.innerHTML;
    window._savedSearchScroll = document.getElementById('search-view')?.parentElement?.scrollTop || 0;
    ...
    try {
        ...
        switchTab('detail'); 
        let scoreStr = data.score || data.info?.skor || data.info?.score || ''; const score = (scoreStr && scoreStr !== '?' && scoreStr !== '0' && scoreStr !== '') ? parseFloat(scoreStr).toFixed(2) : '';
        ...
        // Ambil semua genre, theme, demographic
        const rawGenre = data.genre || data.info?.genre || '';
        const rawTheme = data.theme || data.info?.theme || '';
        const rawDemographic = data.demographic || data.info?.demographic || '';
        const allGenreTags = [...new Set([rawGenre, rawTheme, rawDemographic].join(',').split(',').map(g => g.trim()).filter(g => g.length > 0))];
        const ALLOWED_GENRES = new Set(["Action","Adventure","Comedy","Drama","Ecchi",
            "Fantasy","Game","Girls Love","Boys Love","Gore","Harem","Historical",
            "Horror","Isekai","Josei","Magic","Mahou Shoujo","Martial Arts","Mecha",
            "Military","Music","Mystery","Parody","Psychological","Racing","Reincarnation",
            "Romance","Samurai","School","Sci-Fi","Seinen","Shoujo","Shounen",
            "Slice Of Life","Space","Sports","Super Power","Supernatural","Suspense",
            "Thriller","Time Travel","Vampire","Video Game"]);
        const filteredGenreTags = allGenreTags.filter(g => ALLOWED_GENRES.has(g));
        const genreChipsHtml = ''; // Genre sudah inline di samping rating
        let newestEpUrl = data.episodes.length > 0 ? data.episodes[0].url : ''; let newestEpNum = '?';
        if (data.episodes.length > 0) {
            // Find the highest episode number across all episodes
            let maxNum = 0;
            let maxUrl = data.episodes[0].url;
            data.episodes.forEach(ep => {
                let num = 0;
                if (ep.title) {
                    let m = ep.title.match(/(?:Episode|Eps|Ep)\s*(\d+(\.\d+)?)/i);
                    if (m) { num = parseFloat(m[1]); }
                    else { let nums = ep.title.match(/\d+/g); if (nums) num = parseFloat(nums[nums.length - 1]); }
                }
                if (num > maxNum) { maxNum = num; maxUrl = ep.url; }
            });
            newestEpNum = maxNum > 0 ? String(maxNum % 1 === 0 ? Math.floor(maxNum) : maxNum) : String(data.episodes.length);
            newestEpUrl = maxUrl;
        }
        ...
        console.error(err);
        // Log ke DB supaya bisa dicek di MongoDB
        try {
            fetch(`${API_BASE}/log-failed`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ url, title: '', reason: err?.message || 'fetch error' })
            });
        } catch (_) {}
    } finally { loader(false); }
...
async function loadVideo(url) {
    ...
    try {
        ...
        document.getElementById('watch-view').innerHTML = `
            <div class="video-container-fixed" style="position:relative;">
                <button class="watch-back-btn" onclick="backToDetail()"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M19 12H5M12 19l-7-7 7-7"/></svg></button>
                ${data.streams.length > 0
                    ? `<iframe id="video-player" src="${data.streams[0].url}" allowfullscreen allow="autoplay; fullscreen; encrypted-media" frameborder="0" scrolling="no" style="position:absolute;top:0;left:0;width:100%;height:100%;border:none;display:block;" onload="document.getElementById('video-loading-overlay') && (document.getElementById('video-loading-overlay').style.opacity='0', setTimeout(()=>{let o=document.getElementById('video-loading-overlay');if(o)o.style.display='none';},500))"></iframe>
                       <div id="video-loading-overlay" style="position:absolute;inset:0;background:#000;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:5;pointer-events:none;transition:opacity 0.5s;">
                           <div style="width:40px;height:40px;border:3px solid rgba(59,130,246,0.2);border-top-color:#3b82f6;border-radius:50%;animation:spin 1s linear infinite;margin-bottom:12px;"></div>
                           <div style="color:#a1a1aa;font-size:13px;font-weight:600;">Memuat video...</div>
                       </div>`
                    : `<div style="position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;background:#0a0a0a;">
                           ...
                           <div style="color:#ef4444;font-size:14px;font-weight:800;margin-bottom:8px;text-align:center;">Stream Tidak Tersedia</div>
                           <div style="color:#6b7280;font-size:12px;margin-bottom:20px;text-align:center;line-height:1.5;padding:0 20px;">Server sedang mencari sumber video.<br>Coba lagi dalam beberapa saat.</div>
                           <button onclick="retryLoadStream('${url}')" style="background:#3b82f6;color:#fff;border:none;padding:10px 24px;border-radius:20px;font-size:13px;font-weight:800;cursor:pointer;">🔄 Coba Lagi</button>
                           <div id="video-player" style="display:none;"></div>
                       </div>`
                }
            </div>
...
window.addEventListener('popstate', (e) => { 
    ...
    if (isSentinel) {
        ...
        if (currentPage === 'home' && stack.length <= 1) {
            ...
            setTimeout(() => {
                switchTab(prevPage);
                // FIX v44: restore search content kalau balik ke search
                if (prevPage === 'search' && window._savedSearchContent) {
                    const sc = document.getElementById('search-view');
                    if (sc) {
                        sc.innerHTML = window._savedSearchContent;
                        setTimeout(() => { if (sc.parentElement) sc.parentElement.scrollTop = window._savedSearchScroll || 0; }, 50);
                    }
                }
            }, 50);
        }
...
window.goBack = function() {
    // FIX v44: back yang cerdas + restore konten search
    const stack = window.appPageStack;
    if (stack[stack.length - 1] === 'detail') window.appPageStack.pop();
    const prev = window.appPageStack[window.appPageStack.length - 1] || 'home';
    if (window.appPageStack.length === 0) window.appPageStack = ['home'];
    switchTab(prev);
    // Restore konten search view kalau balik ke search
    if (prev === 'search' && window._savedSearchContent) {
        const sc = document.getElementById('search-view');
        if (sc) {
            sc.innerHTML = window._savedSearchContent;
            // Restore scroll position
            setTimeout(() => {
                if (sc.parentElement) sc.parentElement.scrollTop = window._savedSearchScroll || 0;
            }, 50);
        }
    }
};
...
// ==========================================
window.cachedScheduleData = null; window.cachedJadwalByDay = {}; 
...
function injectScheduleStyles() {
    ...
    style.innerHTML = `
        .sched-day-scroll { display: flex; overflow-x: hidden; padding: 12px 8px; background: #050505; border-bottom: 1px solid #1a1a1a; position: sticky; top: 0; z-index: 10; justify-content: space-between; }
        .sched-day-scroll::-webkit-scrollbar { display: none; }
        .sched-day-item { display: flex; flex-direction: column; align-items: center; gap: 5px; color: #666; font-weight: 700; cursor: pointer; flex: 1; transition: 0.2s; }
        .sched-day-item .s-name { font-size: 11px; }
        .sched-day-item .s-date { font-size: 15px; width: 34px; height: 34px; display: flex; align-items: center; justify-content: center; border-radius: 50%; }
        ...
        .sched-day-item .s-dot { width: 6px; height: 6px; border-radius: 50%; background: #3b82f6; opacity: 0; margin-top: -3px; }
        .sched-day-item.today-marker .s-dot { opacity: 1; }
        .sched-card { display: flex; gap: 0; border: 1px solid #1e1e1e; border-radius: 14px; cursor: pointer; transition: 0.2s; background: #111113; align-items: stretch; margin: 0 12px 10px 12px; box-sizing: border-box; overflow: hidden; }
        .sched-card:active { border-color: #3b82f6; transform: scale(0.985); }
        .sched-left-accent { width: 4px; background: #f59e0b; flex-shrink: 0; border-radius: 0; }
        .sched-left-accent.accent-late { background: #ef4444; }
        .sched-left-accent.accent-wait { background: #444; }
        .sched-inner { display: flex; gap: 12px; padding: 12px; align-items: center; flex: 1; min-width: 0; }
        .sched-time { font-size: 17px; font-weight: 900; color: #fff; width: 52px; text-align: center; flex-shrink: 0; line-height: 1.2; }
        .sched-img { width: 70px; height: 95px; border-radius: 8px; object-fit: cover; border: 1px solid #1e1e1e; flex-shrink: 0; background: #111; object-position: top; }
        .sched-info { flex: 1; min-width: 0; display: flex; flex-direction: column; justify-content: center; gap: 4px; }
        .sched-title { font-size: 16px; font-weight: 800; color: #fff; margin-bottom: 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; line-height: 1.35; }
        .sched-ep { font-size: 14px; color: #d1d5db; font-weight: 600; }
        .sched-stats { font-size: 13px; color: #a1a1aa; display: flex; align-items: center; gap: 6px; font-weight: 500; }
        .sched-status { font-size: 13px; font-weight: 700; display: flex; align-items: center; gap: 5px; margin-top: 2px; }
        .status-wait { color: #888; } .status-wait::before { content: ''; display: inline-block; width: 7px; height: 7px; background: #555; border-radius: 50%; margin-right: 2px; }
        .status-done { color: #f59e0b; } .status-done::before { content: ''; display: inline-block; width: 7px; height: 7px; background: #f59e0b; border-radius: 50%; margin-right: 2px; box-shadow: 0 0 5px rgba(245,158,11,0.7); }
        .status-late { color: #ef4444; } .status-late::before { content: ''; display: inline-block; width: 7px; height: 7px; background: #ef4444; border-radius: 50%; margin-right: 2px; }
        .sched-inline-nav { display: flex; justify-content: space-between; align-items: center; padding: 10px 16px; background: #0a0a0a; border-top: 1px solid #1a1a1a; flex-shrink: 0; }
        .sched-nav-btn { background: #1a1a1a; color: #fff; border: none; padding: 11px 18px; font-size: 14px; font-weight: 800; cursor: pointer; display: flex; align-items: center; gap: 8px; border-radius: 10px; transition: background 0.2s; }
        .sched-nav-btn:active { background: #2a2a2a; }
    `;
    document.head.appendChild(style);
}
...
function renderJadwalDays(activeDay) {
    ...
    for(let i = 0; i < 7; i++) {
        ...
        let isTodayClass = (i === currentDayOfWeek) ? 'today-marker' : '';
        html += `<div class="sched-day-item ${isActive} ${isTodayClass}" onclick="setJadwalDay(${i})"><div class="s-name">${NAMA_HARI[i]}</div><div class="s-date">${tgl}</div><div class="s-dot"></div></div>`;
    }
...
async function loadJadwalData(dayIndex) {
    ...
    try {
        ...
        if (window.cachedScheduleData) { 
            ...
            const res = await fetchMem(`${API_BASE}/latest`, 10000); 
            data = await res.json(); 
        ...
        // 3. URUTKAN DARI PAGI KE MALAM, prioritaskan yang lastUpdate terbaru
        todaysAnime.sort((a, b) => {
            // Sama jam → yang lastUpdate lebih baru duluan
            if (a.releaseHour !== b.releaseHour) return a.releaseHour - b.releaseHour;
        ...
        todaysAnime.forEach((anime, idx) => {
            ...
            let statusText = isReleased ? `<span class="status-done">Sudah Tayang</span>` : `<span class="status-wait">Menunggu Update Baru</span>`;
            let accentClass = isReleased ? '' : 'accent-wait';
            ...
            <div class="sched-card" onclick="loadDetail('${anime.url}')">
                <div class="sched-left-accent ${accentClass}"></div>
                <div class="sched-inner">
                    ...
                    <div class="sched-info">
                        <div class="sched-title">${anime.titleEnglish||anime.title}</div>
                        ...
                        <div class="sched-stats">
                            ...
                            <span style="color:#fbbf24;">⭐ ${mockScore}</span>
                        </div>
...
async function checkAnimeUpdates() {
    try {
        ...
        const res = await fetchMem(`${API_BASE}/latest`, 10000); if (!res || !res.ok) return; const latestData = await res.json();

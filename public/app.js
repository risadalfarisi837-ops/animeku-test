        ...
        db.ref('users/' + currentUser.uid).on('value', async snap => {
            try {
                    ...
                    <div style="position: relative; width: 100%; z-index: 1;">
                        ...
                        <div class="profile-header" style="padding-top: 40px; display:flex; flex-direction:column; align-items:center;">
                            ...
                            <div class="profile-badges" style="display:flex; gap:8px; justify-content:center;">
                                ...
                                <span class="c-badge" style="background: rgba(255,255,255,0.05); color: #a1a1aa; font-size:11px;">${shortUid}</span>
                            </div>
                        </div>
                ...
                db.ref('comments').once('value').then(commentsSnap => {
                    let myComments = [];
                    commentsSnap.forEach(epSnap => { epSnap.forEach(cSnap => { 
                        let c = cSnap.val(); 
                        if(c.uid === currentUser.uid) myComments.push({ ...c, date: new Date(c.waktu || Date.now()).toLocaleDateString('id-ID') }); 
                    }); });
                                ...
                                <div style="background: #1c1c1e; border-radius: 16px; padding: 15px; margin-bottom: 15px;">
                                    <div style="display: flex; gap: 12px; margin-bottom: 12px; align-items: center; cursor: pointer;" onclick="loadDetail('${c.url}')">
                                        <div style="position:relative; flex-shrink:0;">
                                            ...
                                            <div style="position:absolute; bottom:-4px; right:-4px; background:#1c1c1e; border-radius:50%; padding:2px;">
                                                <img src="${userFoto}" style="width:16px; height:16px; border-radius:50%; object-fit:cover; display:block;">
                                            </div>
                                        </div>
